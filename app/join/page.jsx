...
export default function JoinPage() {
  ...
  async function leaveOnClose() {
    try {
      if (!auth.currentUser) return
      await matchmaking.leaveQueue({ exam, subject, mode, uid: auth.currentUser.uid })
      if (unsubRef.current) unsubRef.current()
    } catch (e) {}
  }
  ...
  return (
    <div style={{ padding: 18, maxWidth: 900, margin: '0 auto' }}>
      <h2>Join session</h2>
      {!user ? (<div><p>Sign in to join</p><button onClick={login}>Sign in with Google</button></div>) : (
        <>
