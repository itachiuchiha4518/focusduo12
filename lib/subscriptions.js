...
export function remainingForMode(profile, mode) {
  ...
  return Number(profile?.[field] ?? getPlanDefinition('free')[field === 'freeGroupRemaining' ? 'freeSessionsGroup' : 'freeSessionsOneOnOne'])
}
...
export async function ensureUserProfile(authUser) {
  ...
  if (!snap.exists()) {
    // Generate a referral code for new users: first 8 chars of uid uppercased
    ...
    const fresh = {
      ...
      referredBy: null,         // ← who referred them
  ...
  if (!paidActive) {
    // Plan expired — downgrade back to free
    if ((data.planType || 'free') === 'paid') {
      ...
      // NOTE: streak is NOT reset here — we reset it separately in incrementStreak
      // so that expiry → streak loss happens on the NEXT session attempt (more dramatic)
    }
...
// ─────────────────────────────────────────────────────
// 🔥 STREAK SYSTEM
// Called at the END of every session (from session page)
...
export async function incrementStreak(uid) {
  ...
  return await runTransaction(db, async tx => {
    ...
    const data = snap.data()
    const today = new Date().toISOString().split('T')[0]   // "2025-03-26"
    const lastDate = data.lastStudyDate || null

    let newStreak = data.streakDays ?? 0

    if (lastDate === today) {
      // Already studied today — streak stays, just return current
      return newStreak
    }
...
// Call this ONCE per new user, from the sign-up flow
// Both referrer and new user get 3 bonus sessions
export async function applyReferralCode(newUserUid, referralCode) {
  ...
  return await runTransaction(db, async tx => {
    ...
    const BONUS = 3   // sessions each person gets

    // Update new user — mark as referred, add bonus sessions
    ...
    // Update referrer — add bonus sessions + increment referred count
...
export async function consumeFreeCredit({ uid, mode, sessionId }) {
  ...
  await runTransaction(db, async tx => {
    ...
    if (billingSnap.exists()) return   // already charged
...
export async function createSubscriptionRequest({
  uid, name, email, planId, planLabel,
  amount, utr, upiId, qrText, qrImageUrl
}) {
